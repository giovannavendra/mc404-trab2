...
.data
  ...
  svc_handler_array: .skip 4*8
    ...
    SVC_STACK:
		.skip 4096
		USR_STACK:		@Nao faz diferenca alocar a pilha aqui, pois a seguranca do programa sera comprometida de qualquer forma.
...
RESET_HANDLER:
  ...
  SET_TZIC:
      ...
      msr CPSR_c, #USR_MODE|NO_FIQ  @Troca para o modo usuario
      ldr sp, =USR_STACK						@Inicializa a pilha do usuario
      .set LOCO_ADDRESS, 0x77812000
      ldr pc, =LOCO_ADDRESS         @Salta para o codigo do usuario.
...
IRQ_HANDLER:
    ...
    str r0, [r1, #GPT_SR]
		...
		@Verifica se há callbacks de proximidade.
    ...
    exceeded_threshold:
      ...
      str r1, [r0]
      lsl r8, #2
      ldr r2, [r6, r8]   						@guarda na memoria o endereco para qual deve-se saltar na instrucao sudo.
      ldr r0, =sudo_branch_addr
      ldr r1, =end_callback_loop
      str r1, [r0]
      msr CPSR_c, #USR_MODE|NO_INT  @Muda para o modo usuario antes de chamar a funcao de callback
      blx r2
      mov r7, #23
      svc 0x0												@Syscall que volta para o modo IRQ após retornar da callback.
    end_callback_loop:


		@Verifica se há alarmes registrados.
    ldr r0, =CONTADOR
    ldr r0, [r0]
    ldr r4, =alarm_array
    ...
    found_alarm:
      ...
      str r1, [r0]	@armazena o endereco para o qual a syscall sudo deve saltar quando voltar ao modo IRQ.
    ...
    end_irq_handler:
      ldr r0, =CONTADOR
      ldr r1, [r0]
      add r1, r1, #1  	@Incrementa o contador de tempo.
      str r1, [r0]
...
SVC_HANDLER:
  ...
  msr CPSR_c, #SVC_MODE|NO_INT   @Desabilita interrupcoes.

  ldr r4, =svc_handler_array
  sub r7, r7, #16
  lsl r7, #2
  ldr r4, [r4, r7]
  blx r4												@Salta para o rotulo da syscall que deseja-se executar.
  pop {r4-r12, lr}
  movs pc, lr

@Syscall que retorna para o modo IRQ apos completar uma callback.
sudo:
  pop {r4-r12, lr}							@Desempilha registradores do modo SVC.
  msr CPSR_c, #IRQ_MODE|NO_INT	@Troca para o modo IRQ
  ldr r0, =sudo_branch_addr			@Carrega em r0 o endereco de retorno para IRQ.
  ldr r0, [r0]
  mov pc, r0
...
read_sonar:
  ...
  bhi error_read_sonar
    ...
    bic r6, r6, r0 @limpa o sonar_mux e limpa o sonar_data
    lsl r4, #2
